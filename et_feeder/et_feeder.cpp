...
ETFeeder::ETFeeder(string filename)
  ...
  // cout << "[DEBUG et_feeder] start parsing chakra trace:" << filename << "\n";
  readGlobalMetadata();
  readNextWindow();
}
...
shared_ptr<ETFeederNode> ETFeeder::readNode() {
  ...
  if (!trace_.read(*pkt_msg)) {
    // cout << "[DEBUG et_feeder] node is nullptr\n";
    return nullptr;
  }
  shared_ptr<ETFeederNode> node = make_shared<ETFeederNode>(pkt_msg);
  // cout << "[DEBUG] chakra read node " << node->id() << endl;
...
void ETFeeder::readNextWindow() {
  ...
  do {
    ...
    // cout << "[DEBUG et_feeder] new node " << new_node << "\n";
    if (new_node == nullptr) {
      et_complete_ = true;
      break;
    }
